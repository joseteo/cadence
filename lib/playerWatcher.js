...
export class PlayerWatcher {
    ...
    constructor(onChange) {
        ...
        this._PlayerProxy = Gio.DBusProxy.makeProxyWrapper(playerIface);
        this._RootProxy = Gio.DBusProxy.makeProxyWrapper(rootIface);
        this._BusProxy = Gio.DBusProxy.makeProxyWrapper(busIface);
    }
    ...
    start() {
        ...
        this._bus = new this._BusProxy(Gio.DBus.session, 'org.freedesktop.DBus', '/org/freedesktop/DBus');
    ...
    _addPlayer(busName) {
        ...
        const proxy = new this._PlayerProxy(Gio.DBus.session, busName, MPRIS_PATH);
        this._proxies.set(busName, proxy);

        const root = new this._RootProxy(Gio.DBus.session, busName, MPRIS_PATH);
        this._rootProxies.set(busName, root);

        proxy.connectObject('g-properties-changed', (p) => {
    ...
    _removePlayer(busName) {
        this._proxies.get(busName)?.disconnectObject(this);
