...
import { StyleClass, Palette, Icon, DockBackdrop, TextRgb } from './theme.js';
...
// Contrast the card aims for against its text. Above the 4.5:1 WCAG minimum,
// because the artist line is small and thin enough that antialiasing costs it
// roughly a third of the nominal ratio.
const MIN_CONTRAST = 8.5;
...
class TrackCard extends St.BoxLayout {
    ...
    _build() {
        ...
        this._artClick.connectObject('clicked', () => this._controller?.toggle(), this);
    ...
    _wireButtons() {
        this._playButton.connectObject('clicked', () => this._controller?.toggle(), this);
        this._nextButton.connectObject('clicked', () => this._controller?.next(), this);
        this._prevButton.connectObject('clicked', () => this._controller?.previous(), this);
    }
    ...
    _wireClick() {
        ...
        this._cardClick.connectObject('clicked', () => {
    ...
    _applyColours(colour) {
        // Pick the text colour from what the card actually looks like on screen,
        // not from the cover colour alone: the card is painted at
        // background-opacity over the dark dock, so a pale cover at low opacity
        // still reads as dark and needs white text. Whichever of black/white
        // contrasts better against that blend wins.
        const onDark = !CoverArt.prefersDarkText(
            this._visibleBackground(colour),
            TextRgb.Dark,
            TextRgb.Light
        );

        // Then move the tint far enough to clear the contrast target, so the small
        // artist line is readable too and not just the bolder title.
        const tinted = this._tintFromArt()
            ? CoverArt.ensureContrast(
                colour,
                this._opacity(),
                DockBackdrop,
                onDark ? TextRgb.Light : TextRgb.Dark,
                MIN_CONTRAST
            )
            : colour;

        this._applyBackground(tinted);

        this._setTextStyle(this._title, onDark ? Palette.TitleOnDark : Palette.TitleOnLight, this._titleFontSize, onDark);
        this._setTextStyle(this._artist, onDark ? Palette.ArtistOnDark : Palette.ArtistOnLight, this._artistFontSize, onDark);
        this._applyIconColour(onDark);
        this._onDark = onDark;
    }

    // The playback glyphs sit on the same tinted card as the labels, so they follow
    // the same light/dark decision — white icons vanish on a pale cover.
    _applyIconColour(onDark) {
        const colour = onDark ? Palette.TitleOnDark : Palette.TitleOnLight;
        for (const icon of [this._playIcon, this._pauseIcon, this._prevIcon, this._nextIcon, this._overlay]) {
            icon?.set_style(`color: ${colour};`);
        }
    }

    // The colour the eye sees for the card: the tint (or the neutral fill when the
    // tint is off) composited over the dock background at the configured opacity.
    _visibleBackground(colour) {
        return this._tintFromArt()
            ? CoverArt.compositeOver(colour, this._opacity(), DockBackdrop)
            : CoverArt.compositeOver(Palette.NeutralRgb, Palette.NeutralAlpha, DockBackdrop);
    }
    ...
    // A mid-grey cover gives black and white text almost the same contrast, so the
    // better choice still isn't a comfortable read. The shadow (dark behind light
    // text, light behind dark text) adds a hard edge that keeps the label legible
    // whatever the cover does.
    _setTextStyle(label, colour, fontSize, onDark = true) {
        const shadow = onDark
            ? 'text-shadow: 0 1px 2px rgba(0, 0, 0, 0.6);'
            : 'text-shadow: 0 1px 2px rgba(255, 255, 255, 0.55);';
        label.set_style(`color: ${colour}; font-size: ${fontSize}px; ${shadow}`);
    }
