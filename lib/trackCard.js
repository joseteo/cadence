...
import { StyleClass, Palette, Icon, DockBackdrop } from './theme.js';
import * as CoverArt from './coverArt.js';
import * as ColourPalette from './palette.js';
...
class TrackCard extends St.BoxLayout {
    ...
    _colourFromArt(pixbuf) {
        ...
        // The accent from the sleeve, used for the labels when it is distinct enough
        // from the card to stay readable. Null for monochrome artwork.
        this._secondary = CoverArt.secondaryColour(pixbuf, this._lastColour);
        this._applyColours(this._lastColour);
    }
    ...
    _applyColours(colour) {
        ...
        // Everything is decided against the composite - the tint as it is actually
        // painted over the dock at the current opacity - because that, not the cover
        // colour, is what the labels sit on.
        const card = this._visibleBackground(colour);
        const ink = ColourPalette.inkFor(card, this._tintFromArt() ? this._secondary : null);
        const artistInk = ColourPalette.secondaryInkFor(card, ink);

        this._setTextStyle(this._title, ColourPalette.toCss(ink), this._titleFontSize);
        this._setTextStyle(this._artist, ColourPalette.toCss(artistInk), this._artistFontSize);
        this._applyIconColour(ColourPalette.toCss(ink));
    }

    // The glyphs are read the same way as the title, so they take the same ink.
    _applyIconColour(css) {
        for (const icon of [this._playIcon, this._pauseIcon, this._prevIcon, this._nextIcon, this._overlay]) {
            icon?.set_style(`color: ${css};`);
        }
    }
    ...
    // No shadow: the ink is chosen to clear its contrast target against the card on
    // its own, so the label does not need an outline to be readable.
